...
class Profile extends Component {
    ...
    if (userData.user_id != null) {
      Axios.get(`http://localhost:8000/user/${userData.user_id}`)
        .then((response) => {
          ...
          this.setState({ userData: response.data,h5Name: response.data.name, loading: false });
          ...
          Axios.get(`http://localhost:8000/orders/${userData.user_id}`) // 发送请求以获取订单数据
  .then((ordersResponse) => {
    // 更新状态以存储订单数据
    this.setState({ ordersData: ordersResponse.data });

    // 透過 orders_id 撈出 order_details 資料
    ordersResponse.data.forEach(order => {
      Axios.get(`http://localhost:8000/order_details/${order.orders_id}`)
        .then((orderDetailsResponse) => {
          // 更新状态以存储订单详情数据
          this.setState(prevState => ({
            orderDetailsData: [...prevState.orderDetailsData, orderDetailsResponse.data]
          }));
        })
        .catch((orderDetailsError) => {
          console.error("Failed to fetch order details data:", orderDetailsError);
        });
    });
  })
  .catch((ordersError) => {
  ...
  handleForm1Submit = (event) => {
    ...
    // 更新用户数据
    Axios.post(`http://localhost:8000/updateUserData/${userData.user_id}`, userData)
        .then(() => {
            // 更新用户数据成功后重新获取最新用户数据
            Axios.get(`http://localhost:8000/user/${userData.user_id}`)
                .then((response) => {
...
handleForm2Submit = (event) => {
  ...
  Axios.post("http://localhost:8000/verifyPassword", { userId, oldPassword })
    .then((response) => {
      Axios.post("http://localhost:8000/changePassword", { userId, newPassword })
        .then((response) => {
          alert("密碼修改成功");
        })
        .catch((error) => {
  ...
  handleDateChange = (date) => {
    ...
    const formattedDate = new Date(date.getTime() + (8 * 60 * 60 * 1000)).toISOString().split('T')[0]; // 加上 8 小時後轉換為台北時間
            ...
            <div className="row">
              <div className="col-2 mt-4 profilecol2">
                <div className="row mb-3">
                  ...
                  <img id='userimg' src={userData.user_img ? `/img/users/${userData.user_img}` : "/img/users/Ledian.png"} alt='memberHeadshot' className='w-100 img-fluid mt-1 mx-1 rounded-circle border'></img>
              ...
              <div className="col-10 profilecol10">
                  ...
                  <div className="col-6 mt-1" style={{ display: displayCol6 }}>
                    ...
                    <span className="d-flex align-items-center ">
                      <img
                        ...
                        alt=" "
                      />
                      <h6 className="mb-0 mx-2 fs-6 mt-0">
                        {userData.points}
                      </h6>
                    </span>
                  </div>
                ...
                <div id="minicol10nav"  className="row d-flex align-items-center py-3 mb-3 minisize">
                  <ul id="col10nav"  className="nav fs-5">
                    <div className="col">
                      <li className="nav-item col d-flex justify-content-center">
                        <a className="nav-link active px-0" id="col10Profile-v-tab" data-bs-toggle="tab" href="#Profile-v" aria-selected="true">個人檔案</a>
                      </li>
                    </div>

                    <div className="col">
                      <li className="nav-item col d-flex justify-content-center">
                        <a className="nav-link px-0" id="col10Management-v-tab" data-bs-toggle="tab" href="#Management-v" aria-selected="false">帳號管理</a>
                      </li>
                    </div>

                    <div className="col">
                      <li className="nav-item col d-flex justify-content-center">
                        <a className="nav-link px-0" id="col10Historical_Orders-v-tab" data-bs-toggle="tab" href="#Historical_Orders-v" aria-selected="false">歷史訂單</a>
                      </li>
                    </div>

                    <div className="col">
                      <li className="nav-item col d-flex justify-content-center">
                        <a className="nav-link px-0" id="col10Barcode-v-tab" data-bs-toggle="tab" href="#Barcode-v" aria-selected="false">載具存取</a>
                      </li>
                    </div>
                  </ul>
                </div>
